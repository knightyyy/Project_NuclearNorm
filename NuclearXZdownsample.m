...
N_random=floor(Nx*0.1);
Numberarray_Nu_non=sort([1 randperm(Nx-3,N_random-2) Nx-2]);%随机产生位置
Nx=length(Numberarray_Nu_non);
Nc=67;%表示要插值结果的数目
TRX_pos=TRX_pos_origin(Numberarray_Nu_non(1:end));
%% 扫频设置
...
for index_f=1:Nf
    ...
    Kx(:,index_f)=kxorign(Numberarray_Nu_non(1:end));%将方位维波数域均匀平分
...
%% 进行相位补偿处理
Phasecomp=Kzvec*TRZ;
s_comp_Nu_non=S.*exp(j*Phasecomp);
% [M_comp,N_comp]=size(s_comp_Nu_non);
% nn=2;%二阶
% d0=60;
% m_comp=fix(M_comp/2);
% n_comp=fix(N_comp/2);
...
%         end
%         s_comp_Nu_non(index_M,index_N)=h_comp* s_comp_Nu_non(index_M,index_N);
%     end
% end
%% 进行二维傅里叶变换
for index_f=1:Nf    
    kxvec=Kxvec(:,index_f);
    temp=s_comp_Nu_non(:,index_f);
...
save('D:\yyy\核范数成像\实部虚部仿真1\s_comp_Nu_non.mat','s_comp_Nu_non');
save('D:\yyy\核范数成像\实部虚部仿真1\Numberarray_Nu_non.mat','Numberarray_Nu_non');